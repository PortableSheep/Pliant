...
export type RuleFn<TOptions = unknown, TValue = unknown, TData = Record<string, unknown>> = (
  value: TValue,
  ctx: RuleContext<TData>,
  options: TOptions
) => RuleResult;

export type AsyncRuleFn<TOptions = unknown, TValue = unknown, TData = Record<string, unknown>> = (
  value: TValue,
  ctx: RuleContext<TData>,
  options: TOptions
) => Promise<RuleResult>;
